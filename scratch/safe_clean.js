...
// 1. Remove the entire `khitan-9` CSS block in the <style> tag.
const cssStart = 'if (activeTheme === "khitan-9") {\\n      css += `\\n        /* Khitan-9 Space Theme HUD Overrides */';
// Find the end of this block which is the closing backtick and curly brace
const cssBlockRegex = /if \(activeTheme === "khitan-9"\) \{\s*css \+= `[\s\S]*?\n      \s*`;\n    \}/;
content = content.replace(cssBlockRegex, '');
...
content = content.replace(/\s*\{\s*id:\s*"9[123]",\s*url:\s*"\/templates\/khitan9?[^}]+\},?/g, '');
