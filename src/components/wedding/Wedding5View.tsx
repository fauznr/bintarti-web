...
export default function Wedding5View({
  ...
  const safeGalleryImages = (invitationData?.gallery_images && Array.isArray(invitationData.gallery_images) && invitationData.gallery_images.length > 0)
    ? invitationData.gallery_images
    : fallbackGallery;
  // Dynamic Variables
  const groomNameFull = invitationData?.full_name?.split("&")[0]?.trim() || "Farhan Mahendra";
  const brideNameFull = invitationData?.full_name?.split("&")[1]?.trim() || "Nabila Zhafira";
  ...
  const quoteText = weddingNotes?.quoteText || "{quoteText}";
  const quoteSource = weddingNotes?.quoteSource || "{quoteSource}";
  ...
  // Background photos for dynamic crossfade slideshow (Dynamic Customer Photos with Fallbacks)
  ...
  return (
    <div className="min-h-screen bg-[#FAF6F0] text-[#2A1E17] font-adea-montserrat relative overflow-x-hidden">
      ...
      {(!isOpened || isClosingCover) && (
        <div id="cover-section" className={`fixed inset-0 z-50 flex flex-col items-center justify-between p-6 bg-[#1A120C] text-center select-none overflow-hidden transition-all duration-700 ease-in-out ${
        ...
        <div className="w-full max-w-[430px] mx-auto bg-[#FAF6F0] min-h-screen shadow-2xl relative border-x border-[#3E2312]/20 pb-24 overflow-hidden batik-pattern">
          ...
          <section className="w-full min-h-[90dvh] flex flex-col items-center justify-between p-6 relative z-10">
            ...
            <div className="relative z-10 pt-4 flex items-center gap-2">
              <span className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-white/70 backdrop-blur-xl text-[#3E2312] border border-[#5C3A21]/60 text-[9px] font-black tracking-widest uppercase shadow-md">
                <Sparkles className="w-3 h-3 text-[#5C3A21]" /> PAWIWAHAN AGENG {akadDate ? akadDate.split("-")[0] : "2026"}
              </span>
            ...
            <div className="relative z-10 w-full flex flex-col items-center text-center space-y-4 my-auto">
              <div className="relative w-56 h-72 javanese-arch-top border-4 border-[#5C3A21] p-2 shadow-2xl bg-transparent backdrop-blur-md transform rotate-0 hover:rotate-1 transition-transform">
                <div className="relative w-full h-full javanese-arch-top overflow-hidden border border-[#3E2312]">
                  <Image 
                    src={invitationData?.child_photo_url || weddingNotes?.heroPhotoUrl || "/wedding5-hero.jpg"}
              ...
              <div className="bg-white/60 backdrop-blur-xl rounded-3xl p-6 border-2 border-[#5C3A21]/40 shadow-xl flex flex-col items-center space-y-4 relative">
                <div className="relative w-44 h-44 rounded-full overflow-hidden border-4 border-[#5C3A21] shadow-lg p-1">
                  <div className="relative w-full h-full rounded-full overflow-hidden">
                    <Image 
                      src={weddingNotes?.groomPhotoUrl || "/wedding5-groom.jpg"}
                      alt={groomNameFull}
                      fill
                      className="object-cover object-center"
                    />
                ...
                <div className="space-y-1 text-center">
                  ...
                  >
                    <svg className="w-3 h-3 text-current" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg> @{weddingNotes?.groomInstagram?.replace("@","") || "farhan_mahendra"}
                  </a>
                </div>
              </div>
            </ScrollReveal>
            ...
            <ScrollReveal delay={350}>
              <div className="bg-white/60 backdrop-blur-xl rounded-3xl p-6 border-2 border-[#5C3A21]/40 shadow-xl flex flex-col items-center space-y-4 relative">
                <div className="relative w-44 h-44 rounded-full overflow-hidden border-4 border-[#5C3A21] shadow-lg p-1">
                  <div className="relative w-full h-full rounded-full overflow-hidden">
                    <Image 
                      src={weddingNotes?.bridePhotoUrl || "/wedding5-bride.jpg"}
                      alt={brideNameFull}
                      fill
                      className="object-cover object-center"
                    />
                ...
                <div className="space-y-1 text-center">
                  ...
                  <a 
                    href={`https://instagram.com/${weddingNotes?.brideInstagram?.replace("@","") || "nabilazhafira"}`} 
                    target="_blank" 
                    rel="noopener noreferrer" 
                    className="inline-flex items-center gap-1.5 text-[10px] font-extrabold text-[#3E2312] bg-white/80 backdrop-blur-md px-3.5 py-1.5 rounded-full border border-[#5C3A21] mt-2 hover:bg-[#3E2312] hover:text-white transition-all shadow-sm"
                  >
                    <svg className="w-3 h-3 text-current" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg> @{weddingNotes?.brideInstagram?.replace("@","") || "nabila_zhafira"}
                  </a>
                </div>
              </div>
            </ScrollReveal>
              ...
              {((weddingNotes?.loveStory && weddingNotes.loveStory.length > 0) ? weddingNotes.loveStory : [
                ...
                <ScrollReveal key={idx} delay={idx * 120}>
                  <div className="relative bg-white/60 backdrop-blur-xl p-4 rounded-2xl border border-[#5C3A21]/40 shadow-md space-y-1.5">
                    ...
                    <p className="text-xs text-[#3E2312] leading-relaxed font-adea-lora font-medium">{item.desc || item.description}</p>
                  </div>
                </ScrollReveal>
              ))}
            </div>
          ...
          <section className="px-6 py-10 space-y-6 text-center relative z-10">
            ...
            <div className="grid grid-cols-12 gap-1 sm:gap-2">
              {safeGalleryImages.map((src: string, idx: number) => {
                const galleryImages = safeGalleryImages;
                ...
                <div key={idx} className={colSpan}>
                  <ScrollReveal delay={idx * 80} className="w-full h-full">
                    <div 
                      ...
                      className={`relative w-full ${aspectClass} rounded-2xl overflow-hidden border-2 border-[#5C3A21]/40 shadow-md group cursor-pointer hover:border-[#3E2312] transition-all`}
                    >
                      <Image 
                        src={src} 
          ...
          <section className="px-6 py-12 text-center space-y-6 relative z-10">
            ...
            <ScrollReveal delay={200}>
              <div className="pt-4 border-t border-[#5C3A21]/40 space-y-1">
                ...
                <h3 className="text-3xl font-adea-forum text-[#2A1E17] tracking-wider uppercase font-extrabold">{groomNick} & {brideNick}</h3>
