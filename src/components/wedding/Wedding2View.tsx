...
function ScrollReveal({
  ...
  delay?: number;
  variant?: "fade-up" | "flip-up" | "slide-left" | "slide-right" | "zoom-pop" | "drop"; className?: string;
}) {
            ...
            <div className="grid grid-cols-12 gap-1 sm:gap-2">
              {galleryImages.map((src, idx) => {
                ...
                <div key={idx} className={colSpan}>
                  <ScrollReveal delay={idx * 60} variant={idx % 3 === 0 ? "zoom-pop" : idx % 3 === 1 ? "slide-left" : "slide-right"} className="w-full h-full">
                    <div 
                      onClick={() => setSelectedImage(src)}
                      className={`relative w-full ${aspectClass} rounded-xl overflow-hidden border border-[#D9CDBC] shadow-sm group cursor-pointer hover:opacity-90 transition-all`}
                    >
                      <Image 
                        src={src} 
                        alt={`Gallery ${idx+1}`} 
