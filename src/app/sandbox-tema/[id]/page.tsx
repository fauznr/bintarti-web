...
import Wedding3View from "@/components/wedding/Wedding3View";
import Wedding4View from "@/components/wedding/Wedding4View";
import Wedding5View from "@/components/wedding/Wedding5View";
import Wedding6View from "@/components/wedding/Wedding6View";
import Wedding7View from "@/components/wedding/Wedding7View";
import Wedding8View from "@/components/wedding/Wedding8View";
...
const DEFAULT_CONFIG_THEME_WEDDING_1: ThemeConfig = {
  cover: {
    bgSizeWidth: 100,
    bgSizeHeight: 100,
    bgPosX: 50,
...
const DEFAULT_CONFIG_THEME_AQIQAH_1: ThemeConfig = {
  cover: {
    bgSizeWidth: 100,
    bgSizeHeight: 100,
    bgPosX: 50,
...
export default function ThemePreviewPage() {
  ...
  const [formName, setFormName] = useState("Tamu Undangan");
  ...
  const activeTheme = (normalizedTheme === "wedding-8"
    ? "wedding-8"
    : normalizedTheme === "wedding-7"
    ? "wedding-7"
    : normalizedTheme === "wedding-6"
    ? "wedding-6"
    : normalizedTheme === "wedding-5"
    ? "wedding-5"
    : normalizedTheme === "wedding-4"
    ? "wedding-4"
    : normalizedTheme === "wedding-1"
    ? "wedding-1"
    : normalizedTheme === "wedding-2"
      ? "wedding-2"
      : normalizedTheme === "wedding-3"
        ? "wedding-3"
                        ...
                        : normalizedTheme === "birthday-4"
                          ...
                          : normalizedTheme === "birthday-3" 
                            ...
                            : normalizedTheme === "birthday-2" 
                              ? "birthday-2" 
                              : "birthday-1") as "khitan-1" | "khitan-2" | "khitan-3" | "khitan-4" | "khitan-5" | "khitan-6" | "khitan-7" | "khitan-8" | "birthday-1" | "birthday-2" | "birthday-3" | "birthday-4" | "birthday-5" | "birthday-6" | "birthday-7" | "birthday-8" | "aqiqah-1" | "wedding-1" | "wedding-2" | "wedding-3" | "wedding-4" | "wedding-5" | "wedding-6" | "wedding-7" | "wedding-8";
  ...
  const isWedding = activeTheme === "wedding-1" || activeTheme === "wedding-2" || activeTheme === "wedding-3" || activeTheme === "wedding-4" || activeTheme === "wedding-5" || activeTheme === "wedding-6" || activeTheme === "wedding-7" || (activeTheme && activeTheme.includes("wedding")) || (themeId && themeId.startsWith("wedding_")) || (invitationData && (invitationData.type === "Wedding" || (invitationData.theme && invitationData.theme.toLowerCase().includes("wedding"))));
  const isKhitan = activeTheme === "khitan-1" || activeTheme === "khitan-2" || activeTheme === "khitan-3" || activeTheme === "khitan-4" || activeTheme === "khitan-5" || activeTheme === "khitan-6" || activeTheme === "khitan-7" || activeTheme === "khitan-8";
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (hash && hash.length > 1) {
        let decoded = "";
        try {
          const binaryStr = atob(hash.substring(1));
          const bytes = Uint8Array.from(binaryStr, (c) => c.charCodeAt(0));
          decoded = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
        } catch {
          try {
            decoded = decodeURIComponent(escape(atob(hash.substring(1))));
          } catch {}
        }
        if (decoded && decoded.trim() && !/[\uFFFD\u0000-\u001F\u007F-\u009F]/.test(decoded)) {
          setGuestName(decoded.trim());
          setFormName(decoded.trim());
        }
      }
  ...
  const {
    ...
    const childFullName = (invitationData && invitationData.full_name) ? invitationData.full_name : (isWedding ? "Yoshua & Jessica" : (isKhitan || isAqiqah) ? "Saka Niskala" : activeTheme === "birthday-8" ? "Saka Niskala" : activeTheme === "birthday-5" ? "Putu Gede Satria Wijaya" : "Kanaya Almirha");
    const parentsName = (invitationData && invitationData.parents_name) ? invitationData.parents_name : (isWedding ? "Bapak H. Bambang & Ibu Hj. Retno / Bapak H. Hendra & Ibu Hj. Ratna" : (isKhitan || isAqiqah) ? "Bapak Adrian Mahendra & Ibu Natasha Salsabila" : "Bapak Hendra Pratama & Ibu Sari Dewi");
    const childNickname = (invitationData && invitationData.nickname) ? invitationData.nickname : (isWedding ? "Yoshua & Jessica" : (isKhitan || isAqiqah) ? "Saka" : activeTheme === "birthday-8" ? "Saka" : activeTheme === "birthday-5" ? "Satria" : "Kanaya");
    ...
    } else if ((isKhitan || isAqiqah || !isCustomInvitation) && (!isCustomInvitation || (isCustomInvitation && (!invitationData?.bank_account || invitationData?.bank_account.trim() === "" || invitationData?.bank_account === "[]")))) {
      bankAccounts = [
  ...
  if (themeId !== "khitan-1" && themeId !== "khitan-2" && themeId !== "khitan-3" && themeId !== "khitan-4" && themeId !== "khitan-5" && themeId !== "khitan-6" && themeId !== "khitan-7" && themeId !== "khitan-8" && themeId !== "birthday-1" && themeId !== "birthday-2" && themeId !== "birthday-3" && themeId !== "birthday-4" && themeId !== "birthday-5" && themeId !== "birthday-6" && themeId !== "birthday-7" && themeId !== "birthday-8" && themeId !== "aqiqah-1" && themeId !== "wedding-1" && themeId !== "wedding-2" && themeId !== "wedding-3" && themeId !== "wedding-4" && themeId !== "wedding-5" && themeId !== "wedding-6" && themeId !== "wedding-7" && themeId !== "wedding-8" && !isCustomInvitation) {
    return (
  ...
  if (isWedding && !designerOpen) {
    const wrappedWeddingView = (() => {
      ...
      if (activeTheme === "wedding-5") return <Wedding5View invitationData={demoData} guestName={guestName} themeId={themeId} />;
      if (activeTheme === "wedding-4") return <Wedding4View invitationData={demoData} guestName={guestName} themeId={themeId} />;
      if (activeTheme === "wedding-3") return <Wedding3View invitationData={demoData} guestName={guestName} themeId={themeId} />;
      if (activeTheme === "wedding-2") return <Wedding2View invitationData={demoData} guestName={guestName} themeId={themeId} />;
      return <Wedding1View invitationData={demoData} guestName={guestName} themeId={themeId} />;
    })();
    return (
      <>
        {wrappedWeddingView}
  ...
  return (
    ...
    >
                  ...
                  (() => {
                    ...
                    return (
                      ...
                      onDoubleClick={props.onDoubleClick}
                        className={`${props.className} text-[10px] font-black ${isKhitan ? "bg-white text-slate-800 border border-slate-200" : "text-white"} ${isWedding ? "bg-slate-900/90 text-[#D4AF37] border border-[#C5A059]/50 font-serif" : activeTheme === "birthday-8" ? "bg-[#EF8D21]" : activeTheme === "birthday-7" ? "bg-[#7B68B1]" : activeTheme === "birthday-2" ? "bg-emerald-600/90" : isKhitan ? "" : "bg-pink-500/90"} px-3.5 py-1.5 rounded-full inline-block tracking-wider uppercase shadow-sm cursor-pointer`} 
                        style={props.style}
                      >
        ...
        {isOpened && (
          ...
          >
            <div className={`w-5 h-5 flex flex-col items-center justify-center gap-0.5 ${isAutoScrolling ? 'animate-bounce' : ''}`}>
               ...
               <div className={`w-0 h-0 border-l-[4px] border-l-transparent border-r-[4px] border-r-transparent border-t-[6px] border-t-current mt-0.5`}></div>
            </div>
          </button>
        )}
      ...
      {/* ─────────────────────────────────────────────────────────────────
          ...
          ───────────────────────────────────────────────────────────────── */}
      {isOpened && (
        <div className="max-w-[430px] mx-auto space-y-4 animate-fade-in pb-12 pt-4 px-2 sm:px-3">
                ...
                {scheduleLines.length > 0 ? (
                  <>
                    {(() => {
                     ...
                     {!(isKhitan || isAqiqah || isWedding) && activitiesPhoto && (!isCustomInvitation || !!invitationData?.activities_photo_url) && (
                      <div 
                        {...(() => {
          ...
          {showEnvelopeSection && (
            ...
            {renderCustomOrnaments("envelope")}
              ...
              >
                ...
                {/* Bank transfer card */}
                {(bankAccounts.length > 0 || isLegacyBank || themeId === "khitan-1" || themeId === "khitan-2" || themeId === "khitan-3" || themeId === "khitan-4" || themeId === "khitan-5" || themeId === "khitan-6" || themeId === "khitan-7" || themeId === "khitan-8" || themeId === "birthday-1" || themeId === "birthday-2" || themeId === "birthday-3" || themeId === "birthday-4" || themeId === "birthday-5" || themeId === "birthday-6" || themeId === "birthday-7" || themeId === "birthday-8" || themeId === "aqiqah-1" || themeId === "wedding-1" || (invitationData && invitationData.gift_address)) && (
                  <div 
                    {...(() => {
                  ...
                  >
                    {(bankAccounts.length > 0 || isLegacyBank || themeId === "khitan-1" || themeId === "khitan-2" || themeId === "khitan-3" || themeId === "khitan-4" || themeId === "khitan-5" || themeId === "khitan-6" || themeId === "khitan-7" || themeId === "khitan-8" || themeId === "birthday-1" || themeId === "birthday-2" || themeId === "birthday-3" || themeId === "birthday-4" || themeId === "birthday-5" || themeId === "birthday-6" || themeId === "birthday-7" || themeId === "birthday-8" || themeId === "aqiqah-1" || themeId === "wedding-1") && (
          ...
          >
            ...
            >
              {getSortedElements("closing").map((el) => {
                ...
                if (el.id === "parents") {
                  if ((isKhitan || isAqiqah || isWedding) && layoutConfig.closing.hideBody !== true) {
