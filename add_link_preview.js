...
// Insert state
const statePattern = 'const [isFullscreen, setIsFullscreen] = useState(false);';
const stateReplacement = 'const [isFullscreen, setIsFullscreen] = useState(false);\n  const [showLinkPreview, setShowLinkPreview] = useState(true);';
c = c.replace(statePattern, stateReplacement);

// Insert UI component
const uiPattern = `                <div>
                  <label htmlFor="comment"`;
const uiReplacement = `                {/* Link Preview Card */}
                {showLinkPreview && (
                  <div className="relative flex p-1.5 mb-2 rounded-lg border border-slate-200 bg-slate-50/50 overflow-hidden text-left shadow-sm">
                    ...
                    <div className="flex-shrink-0 w-12 h-12 rounded bg-slate-200 overflow-hidden mr-2 flex items-center justify-center relative">
                      <Image 
                        src={invitationData?.layout_config?.cover?.bgUrl || "https://bintarti.store/icon.png"} 
                        alt="Thumbnail" 
                        fill
...
c = c.replace(uiPattern, uiReplacement);

fs.writeFileSync(path, c);
console.log('Successfully added link preview card');
