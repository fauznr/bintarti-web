    ...
    >
        ...
        <div className="py-16 px-6 space-y-8">
          ...
          <Reveal delay={200} className="relative z-10">
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
              {galleryPhotos.map((photo, idx) => {
                const baseRot = idx % 2 === 0 ? (idx % 4 === 0 ? -2.5 : 1.5) : idx % 3 === 0 ? 2 : -1;
                return (
                  <div
                    key={idx}
                    onClick={() => setSelectedImage(photo)}
                    className="cursor-pointer group transition-transform duration-100 hover:scale-105"
                    style={{
                      background: "#F7EDD4",
                      padding: "4px 4px 18px 4px",
                      boxShadow: "-2px -1px 0 rgba(0,0,0,0.2), 2px 3px 8px rgba(0,0,0,0.4)",
                      transform: `rotate(${baseRot + velocity * 0.15}deg) translateY(${velocity * (idx % 2 === 0 ? 0.2 : -0.2)}px)`,
                    }}
                  >
      ...
      <section id="qrcode-section" className="relative -mt-8 py-16 px-6 z-50 text-center" style={{ background: "#24140B" }}>
        ...
        <Reveal className="relative z-10 space-y-6 mt-4 max-w-sm mx-auto">
          ...
          >
            <img
              src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(`WEDDING-GUEST-${guest || "Tamu Undangan"}`)}`}
      ...
      {isOpened && (
        <div className="fixed bottom-6 right-6 z-[990] flex flex-col items-center gap-3">
          ...
          >
            <QrCode className="w-5 h-5 text-amber-200 group-hover:text-amber-100 transition-colors" />
          </button>
